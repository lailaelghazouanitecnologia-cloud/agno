...
pub trait Embedder: Send + Sync {
    ...
    async fn embed_one(&self, text: String) -> Result<Vec<f32>> {
        ...
        results.pop().ok_or_else(|| common_error::Error::new(common_error::ErrorKind::Provider, "No embedding returned"))
    }
}
...
impl OpenAIEmbedderConfig {
    ...
    pub fn from_env() -> Result<Self> {
        let api_key = env::var("OPENAI_API_KEY")
            .map_err(|_| common_error::Error::new(common_error::ErrorKind::Config, "OPENAI_API_KEY not set"))?;
...
impl Embedder for OpenAIEmbedder {
    ...
    async fn embed(&self, texts: Vec<String>) -> Result<Vec<Vec<f32>>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Provider, format!("Request failed: {}", e)))?;

        let status = response.status();
        let body = response
            .text()
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Provider, format!("Failed to read response: {}", e)))?;

        if !status.is_success() {
            return Err(common_error::Error::new(common_error::ErrorKind::Provider, format!("API error ({}): {}", status, body)));
        }

        let embed_response: OpenAIEmbedResponse = serde_json::from_str(&body)
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Provider, format!("Failed to parse response: {}", e)))?;
...
impl CohereEmbedderConfig {
    ...
    pub fn from_env() -> Result<Self> {
        let api_key = env::var("COHERE_API_KEY")
            .map_err(|_| common_error::Error::new(common_error::ErrorKind::Config, "COHERE_API_KEY not set"))?;
        Ok(Self::new(api_key))
    }
...
impl GoogleEmbedderConfig {
    ...
    pub fn from_env() -> Result<Self> {
        let api_key = env::var("GOOGLE_API_KEY")
            ...
            .map_err(|_| common_error::Error::new(common_error::ErrorKind::Config, "GOOGLE_API_KEY not set"))?;
        Ok(Self::new(api_key))
    }
