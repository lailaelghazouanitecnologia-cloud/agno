...
use crate::monologue::{ChangeAction, PlanStep, Severity};
...
impl SimulationEngine {
    ...
    /// Analyze a plan and produce a simulation result.
    /// This is a static analysis — no LLM calls, no actual file changes.
    pub fn analyze_plan(&self, steps: &[PlanStep]) -> SimResult {
        ...
        for step in steps {
            ...
            // Estimate tokens based on depth level
            let step_tokens = estimate_tokens_for_depth(step.depth_level);
    ...
    fn check_step_risks(&self, step: &PlanStep, risks: &mut Vec<SimRisk>) {
        // Risk: creating files that might already exist
        if step.action.to_lowercase().contains("create") {
            for file in &step.files {
                risks.push(SimRisk {
                    description: format!("Creating {} — verify it doesn't already exist", file),
        ...
        for file in &step.files {
            if file.contains("lib.rs") || file.contains("mod.rs") || file.contains("Cargo.toml") {
                risks.push(SimRisk {
                    description: format!("Modifying shared file: {}", file),
    ...
    fn check_ordering_risks(&self, steps: &[PlanStep], risks: &mut Vec<SimRisk>) {
        for (i, step) in steps.iter().enumerate() {
            for dep in &step.depends_on {
                // Check if dependency exists
                let dep_exists = steps.iter().any(|s| &s.name == dep);
                if !dep_exists {
                    risks.push(SimRisk {
                        description: format!(
...
/// Estimate tokens needed to generate code at a given depth level.
fn estimate_tokens_for_depth(depth: u8) -> u32 {
    match depth {
        0 => 2000, // Workspace-level: architecture, many files
        1 => 1500, // Module-level: structural changes
        2 => 800,  // File-level: implementation
        _ => 400,  // Function-level: focused edits
    }
}
...
mod tests {
    ...
    fn sample_plan() -> Vec<PlanStep> {
        vec![
            PlanStep {
                name: "add-trait".into(),
                action: "Create trait in common/".into(),
                files: vec!["common/src/lib.rs".into()],
                depends_on: vec![],
                depth_level: 1,
            },
            PlanStep {
                name: "implement".into(),
                action: "Implement trait in auth/".into(),
                files: vec!["auth/src/validator.rs".into()],
                depends_on: vec!["add-trait".into()],
                depth_level: 2,
            },
            PlanStep {
                name: "add-tests".into(),
                action: "Add tests".into(),
                files: vec!["auth/tests/validator_test.rs".into()],
    ...
    fn detects_shared_file_risk() {
        let plan = vec![PlanStep {
            name: "modify-lib".into(),
            action: "Modify lib.rs".into(),
    ...
    fn go_assessment_for_clean_plan() {
        let clean_plan = vec![PlanStep {
            name: "simple-edit".into(),
            action: "Edit function".into(),
            files: vec!["src/handler.rs".into()],
            depends_on: vec![],
            depth_level: 3,
        }];
