...
use kkr_runtime::{Executor, ExecutorConfig, ExecutorHandle, Scheduler, SchedulerConfig, TaskQueue};
...
async fn test_executor_submit_checked_rejects_after_shutdown() {
    ...
    assert_eq!(
        result.unwrap_err(),
        kkr_runtime::ExecutorError::Shutdown
    );
}
...
fn test_executor_error_display() {
    assert_eq!(
        format!("{}", kkr_runtime::ExecutorError::QueueFull),
        "Task queue is full"
    );
    assert_eq!(
        format!("{}", kkr_runtime::ExecutorError::Shutdown),
        "Executor is shutting down"
    );
    assert_eq!(
        format!("{}", kkr_runtime::ExecutorError::TaskNotFound),
        "Task not found"
    );
}
...
fn test_executor_error_eq() {
    assert_eq!(
        kkr_runtime::ExecutorError::QueueFull,
        kkr_runtime::ExecutorError::QueueFull
    );
    assert_ne!(
        kkr_runtime::ExecutorError::QueueFull,
