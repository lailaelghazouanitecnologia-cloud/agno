...
impl Reranker for CohereReranker {
    ...
    async fn rerank(&self, query: &str, documents: Vec<Document>, top_k: usize) -> Result<Vec<Document>> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Rerank request failed: {}", e)))?;
...
impl JinaReranker {
    ...
    pub fn from_env() -> Result<Self> {
        let api_key = env::var("JINA_API_KEY")
            .map_err(|_| common_error::Error::new(common_error::ErrorKind::Config, "JINA_API_KEY not set"))?;
        Ok(Self::new(api_key))
    }
...
impl VoyageReranker {
    ...
    pub fn from_env() -> Result<Self> {
        let api_key = env::var("VOYAGE_API_KEY")
            .map_err(|_| common_error::Error::new(common_error::ErrorKind::Config, "VOYAGE_API_KEY not set"))?;
        Ok(Self::new(api_key))
    }
