...
use entity_tools::advanced::{AdvancedTool as Tool, ToolCategory, ToolContext, ToolExample, ToolMetadata, ToolSchema};
use common_error::Result;

const DEFAULT_MAX_READ_SIZE: usize = 1024 * 1024;
const DEFAULT_MAX_WRITE_SIZE: usize = 10 * 1024 * 1024;

fn resolve_path(path: &str, ctx: &ToolContext) -> std::result::Result<PathBuf, common_error::Error> {
    if path.is_empty() {
    ...
    let canonical = if resolved.exists() {
        resolved
            .canonicalize()
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to resolve path: {}", e)))?
    } else {
        let parent = resolved.parent().ok_or_else(|| {
            common_error::Error::new(common_error::ErrorKind::Internal, "Invalid path: no parent directory")
        })?;
        if parent.exists() {
            let canonical_parent = parent
                .canonicalize()
                .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to resolve parent: {}", e)))?;
...
impl Tool for ReadFileTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        let metadata = fs::metadata(&path)
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("File not found: {}", e)))?;

        if metadata.len() as usize > self.max_size {
            return Err(common_error::Error::new(
                common_error::ErrorKind::Internal,
                format!("File too large: {} bytes (max {})", metadata.len(), self.max_size),
            ));
        }

        let content = fs::read_to_string(&path)
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to read: {}", e)))?;
...
impl Tool for WriteFileTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        if params.content.len() > self.max_size {
            return Err(common_error::Error::new(
                common_error::ErrorKind::Internal,
                format!("Content too large: {} bytes (max {})", params.content.len(), self.max_size),
            ));
        }
        ...
        if params.create_dirs {
            if let Some(parent) = path.parent() {
                fs::create_dir_all(parent)
                    .await
                    .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to create dirs: {}", e)))?;
            }
        }
        ...
        if params.append {
            ...
            let mut file = tokio::fs::OpenOptions::new()
                ...
                .open(&path)
                .await
                .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to open: {}", e)))?;
            file.write_all(params.content.as_bytes())
                .await
                .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to write: {}", e)))?;
        } else {
...
impl Tool for ListDirTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        let mut read_dir = fs::read_dir(&path)
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to read dir: {}", e)))?;

        while let Some(entry) = read_dir
            .next_entry()
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to read entry: {}", e)))?
        {
...
impl Tool for DeleteTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        let metadata = fs::metadata(&path)
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Path not found: {}", e)))?;
        ...
        if metadata.is_dir() {
            ...
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to delete dir: {}", e)))?;
        } else {
            fs::remove_file(&path)
                .await
                .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to delete file: {}", e)))?;
        }
...
impl Tool for CopyTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        fs::copy(&source, &dest)
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to copy: {}", e)))?;
...
impl Tool for MoveTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        fs::rename(&source, &dest)
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to move: {}", e)))?;
...
impl Tool for MkdirTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to create dir: {}", e)))?;
...
impl Tool for ExistsTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        let path_str = params
            .get("path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| common_error::Error::new(common_error::ErrorKind::Internal, "Missing path"))?;
...
impl Tool for FileInfoTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        let metadata = fs::metadata(&path)
            .await
            .map_err(|e| common_error::Error::new(common_error::ErrorKind::Internal, format!("Failed to get metadata: {}", e)))?;
