...
use kkr_project::{AgentRole, Plan, PlanStep, Project, Template, step_from_legacy};
use tokio::sync::mpsc;
...
impl Coordinator {
    ...
    pub fn create_plan_from_template(&self, template: &Template, task: &str) -> Plan {
        let mut plan = Plan::new(format!("{}: {}", template.name, task), task);
    ...
    pub fn create_plan(&self, task: &str) -> Plan {
        let mut plan = Plan::new(task, task);
        ...
        if file_count == 0 {
            ...
            plan.add_step(step_from_legacy(
                "scaffold",
                &format!("Create project structure for: {}", task),
                AgentRole::Worker,
            ));
            plan.add_step(
                step_from_legacy("implement", "Implement core functionality", AgentRole::Worker)
                    .after("scaffold".to_string()),
            );
            plan.add_step(
                step_from_legacy("validate", "Verify the implementation compiles and works", AgentRole::Validator)
            ...
            plan.add_step(
                step_from_legacy("implement", "Make the required changes", AgentRole::Worker)
                    .after("analyze".to_string()),
            );
            plan.add_step(
                step_from_legacy("refactor", "Propagate changes across codebase", AgentRole::Worker)
                    .after("implement".to_string()),
            );
            plan.add_step(
                step_from_legacy("validate", "Verify changes are correct", AgentRole::Validator)
                    .after("refactor".to_string()),
            );
        }
