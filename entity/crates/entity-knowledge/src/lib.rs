...
impl KnowledgeBuilder {
    ...
    pub fn build(self) -> Result<Knowledge> {
        let embedder = self
            .embedder
            .ok_or_else(|| common_error::Error::new(common_error::ErrorKind::Config, "Embedder is required"))?;
        let vector_store = self
            .vector_store
            .ok_or_else(|| common_error::Error::new(common_error::ErrorKind::Config, "VectorStore is required"))?;
