...
impl Reader for TextReader {
    ...
    async fn read(&self, source: &str) -> Result<Vec<Document>> {
        let content = tokio::fs::read_to_string(source)
            .await
            .map_err(|e| kkr_core::Error::Knowledge(format!("Failed to read file: {}", e)))?;
...
impl Reader for JsonReader {
    ...
    async fn read(&self, source: &str) -> Result<Vec<Document>> {
        ...
        let value: serde_json::Value = serde_json::from_str(&content)
            .map_err(|e| kkr_core::Error::Knowledge(format!("Failed to parse JSON: {}", e)))?;
...
impl Reader for WebReader {
    ...
    async fn read(&self, source: &str) -> Result<Vec<Document>> {
        let response = self
            ...
            .send()
            .await
            .map_err(|e| kkr_core::Error::Knowledge(format!("Failed to fetch URL: {}", e)))?;
