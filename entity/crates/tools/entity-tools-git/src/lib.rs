//! Git tools for the entity agent system.
//!
//! Provides six git operations (status, diff, log, add, commit, branch)
//! implemented via the AdvancedTool trait from entity-tools.
...
impl GitStatusTool {
    ...
    fn resolve_path(&self, path: Option<&str>, ctx: &ToolContext) -> PathBuf {
        path.map(PathBuf::from)
            .or_else(|| ctx.workspace_root.as_ref().cloned())
...
impl Tool for GitStatusTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        ...
        let output = Command::new("git")
            ...
            .output()
            .await
            .map_err(|e| Error::new(ErrorKind::Internal, format!("Failed to run git: {}", e)))?;

        if !output.status.success() {
            return Err(Error::new(
                ErrorKind::Internal,
                format!("Git error: {}", String::from_utf8_lossy(&output.stderr)),
            ));
        }
...
impl Tool for GitDiffTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<Value> {
        let params: DiffParams = serde_json::from_value(params)
            .map_err(|e| Error::new(ErrorKind::Internal, format!("Invalid parameters: {}", e)))?;
