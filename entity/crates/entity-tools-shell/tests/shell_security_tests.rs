...
/// Assert that execute returns a security-related error (Internal kind with security message).
async fn assert_security_error(tool: &dyn Tool, params: serde_json::Value, ctx: &ToolContext) {
    let result = tool.execute(params, ctx).await;
    match result {
        Err(e) => {
            assert!(
                e.kind == common_error::ErrorKind::Internal,
                "Expected Internal (security) error, got: {:?}",
                e
            );
        }
...
async fn assert_path_traversal_error(
    ...
    match result {
        Err(e) => {
            assert!(
                e.kind == common_error::ErrorKind::PathTraversal,
...
/// Assert that execute returns a Validation (InvalidValue) error.
async fn assert_validation_error(
    ...
    match result {
        Err(e) => {
            assert!(
                e.kind == common_error::ErrorKind::InvalidValue,
...
mod env_validation_tests {
    ...
    async fn ld_preload_rejected_in_params() {
        ...
        match result {
            Err(e) => {
                assert_eq!(e.kind, common_error::ErrorKind::Internal);
                assert!(
                    e.message.contains("LD_PRELOAD"),
                    "Error should mention LD_PRELOAD: {}",
                    e.message
                );
            }
            other => panic!("Expected error for LD_PRELOAD, got: {:?}", other),
        }
    }
    ...
    async fn ld_library_path_rejected() {
        ...
        let result = tool
            ...
            .await;
        assert!(matches!(result, Err(ref e) if e.kind == common_error::ErrorKind::Internal));
    }
...
mod working_dir_tests {
    ...
    async fn working_dir_inside_workspace_allowed() {
        ...
        let ctx = ctx_with_workspace(workspace.path().to_str().unwrap());
...
mod edge_case_tests {
    ...
    async fn bash_tool_name() {
        let tool = entity_tools_shell::BashTool::new();
    ...
    async fn timeout_enforced() {
        ...
        let result = tool
            ...
            .await;
        // Should timeout -- returns an Internal error with "timed out" message.
        match result {
            Err(e) => {
                assert_eq!(e.kind, common_error::ErrorKind::Internal);
                assert!(
    ...
    async fn all_tools_returns_three() {
        let tools = entity_tools_shell::all_tools();
