...
use crate::plan::{AgentRole, step_from_legacy};
use kkr_plan::Step;
...
impl Template {
    ...
    pub fn from_kkr(content: &str) -> Result<Self, TemplateError> {
        ...
        let plan: Vec<Step> = raw.plan.into_iter().map(|p| {
            step_from_legacy(&p.name, &p.action, parse_agent_role(&p.agent))
    ...
    pub fn to_kkr(&self) -> String {
        ...
        for step in &self.plan {
            out.push_str("\n[[plan]]\n");
            out.push_str(&format!("name = \"{}\"\n", step.name));
            out.push_str(&format!("agent = \"{}\"\n", agent_role_from_strategy(&step.strategy)));
...
fn agent_role_from_strategy(strategy: &kkr_plan::StepStrategy) -> &str {
    match strategy {
        kkr_plan::StepStrategy::AgentDispatch { agent_role, .. } => agent_role.as_str(),
        _ => "worker",
    }
}
